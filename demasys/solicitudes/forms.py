...
from models import Solicitudes, conceptos, FORMAS_D_PAGO, ArchivoPendiente
...
class SearchForm(forms.Form): ###index
	...
	beneficiario = forms.ModelChoiceField(queryset = empleado.objects.filter(Q(puesto =8)|Q(puesto =9)|Q(puesto=10)|Q(puesto=11)|Q(puesto=12)),required = False) ##Cambio para llamar solo los operadores Activos.
...
class SolicitudForm (forms.ModelForm):  ###Agregar
	...
	beneficiario = forms.ModelChoiceField(queryset=empleado.objects.filter(Q(puesto =8)|Q(puesto =9)|Q(puesto=10)|Q(puesto=11)|Q(puesto=12)), required = False)###Cambiamos la propiedad de objects para filtrar solo los operadores activos.
	sol_bill = forms.ModelChoiceField(queryset = Viaje.objects.filter(fecha_ent__isnull=True, factura__isnull=True ), required= False)
	benef_otros = forms.ModelChoiceField(queryset=Benefi.objects.all(), required = False)
...
class Sol_Edit_1_Form(forms.ModelForm): ##Autorizar
	...
	beneficiario = forms.ModelChoiceField(queryset = empleado.objects.filter(Q(puesto =8)|Q(puesto =9)|Q(puesto=10)|Q(puesto=11)|Q(puesto=12)),required = False)
	sol_bill = forms.ModelChoiceField(queryset = Viaje.objects.filter(fecha_ent__isnull=True, factura__isnull=True), required= False)
	benef_otros = forms.ModelChoiceField(queryset=Benefi.objects.all(), required = False)	
...
class conceptos_form(forms.ModelForm):
	...
	def __init__(self, *args, **kwargs):
		...
		self.fields['comprobante_fiscal'].widget.attrs['class']='form-control'
		self.fields['comprobante_no_fiscal'].widget.attrs['class']='form-control'
...
class archivoPendienteForm(forms.ModelForm):
    ...
    #def __init__(self, *args, **kwargs):
    #    super(archivoPendienteForm, self).__init__(*args, **kwargs)
