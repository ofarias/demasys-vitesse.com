...
def actualizar(request, viaje_id):
    ...
    print "esta es el flujo actual:" + str(current.state.pk)
    template = ''
    view_context = {             
        'viaje': viaje,    
    ...
    #Cancelado
        ...
        print 'Este es el viaje:' + str(viaje_id)
    ...
    if request.method == 'POST':
	    ...
	    operador = request.POST['operador']
            ...
            if h >= ho:
               ...
               if vig > var and (c_opalo is True or c_rc is True)  and (esta == 10 or esta == 9 or esta == 1):  
            #Crear workflow
...
def viajesAbiertos(request):
    ...
    lista_viajes= Viaje.objects.filter(fecha_ent = None,**filters).exclude(factura = 'Cancelado').order_by('-pk')
    print lista_viajes
